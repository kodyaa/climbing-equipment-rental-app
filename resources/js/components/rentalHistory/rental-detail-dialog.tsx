...
import type { Rental } from "./rental-history-table"
      ...
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1">
            ...
            <InfoRow label="Rental Date" value={formatDate(rental.rental_date)} />
            <InfoRow label="Expected Return" value={formatDate(rental.expected_return_date)} />
            {rental.returned_at && (
              <InfoRow
                label="Returned At"
                value={
        ...
        <DialogFooter className="gap-2">
          ...
          {isActionable && (
            <>
              <Button
                variant="destructive"
                onClick={() => { onCancel(rental); onOpenChange(false) }}
                className="cursor-pointer gap-1.5"
              >
                <XCircleIcon className="size-4" /> Cancel Rental
              </Button>
              <Button
                onClick={() => { onReturn(rental); onOpenChange(false) }}
                className="cursor-pointer gap-1.5"
              >
