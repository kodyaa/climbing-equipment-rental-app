...
export function RentalHistoryTable({
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      router.get("/rentals/history", params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
      })
  ...
  const navigatePage = (page: number) => {
    ...
    if (sortBy !== "id") params.sort = sortBy
    if (sortDir !== "desc") params.direction = sortDir
    router.get("/rentals/history", params, { preserveState: true, preserveScroll: true })
  }
            ...
            {rentals.data.length === 0 ? (
              ...
              rentals.data.map((rental) => (
                <TableRow key={rental.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <TableCell className="text-sm">{formatDate(rental.rental_date)}</TableCell>
                  <TableCell className="text-sm">{formatDate(rental.expected_return_date)}</TableCell>
                  <TableCell>
